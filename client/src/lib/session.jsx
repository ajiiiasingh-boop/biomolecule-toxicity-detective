...
export function SessionProvider({ children }) {
  ...
  // When the API is live it owns the score. Pull it once on connect so a
  // player who reloads with the server running gets the server's tally.
  useEffect(() => {
    if (mode !== 'api') return;
    let cancelled = false;
    fetchProgress(sessionId)
      .then(({ session }) => {
        ...
        setState((prev) => ({
          ...prev,
          score: session.score,
          cases: { ...prev.cases, ...session.cases },
          quiz: session.quiz
